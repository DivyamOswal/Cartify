...
export const createOrder = async (req:Request, res: Response)=>{
    ...
    const orderItems = items.map((item: any)=>{
        ...
        return {product: dbProduct.id, name: dbProduct.name, image: dbProduct.image, price: dbProduct.price, quantity: item.quantity, unit: dbProduct.unit}
    })
...
// Get the user's orders GET ( /api/orders )
export const getUserOrders = async (req:Request, res: Response)=>{
    ...
    const orders = await prisma.order.findMany({
        where, include: {deliveryPartner: {select: {name: true, phone: true}}},
