...
const checkLowStock = inngest.createFunction(
    ...
    async ({event, step})=>{
        ...
        return {alerted: true, produc: product.name, stock:product.stock}
    }
)

// Monthly Offers Email (1st of evry moth - payday)
...
const autoAssignRider = inngest.createFunction({
    ...
    const result = await step.run("assign-rider", async ()=>{
        ...
        // Skip if order doesn't exist, already assigned, ord cancelled
